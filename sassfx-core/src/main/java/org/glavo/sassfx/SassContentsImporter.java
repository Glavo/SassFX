...
///
/// When a stylesheet loaded by an importer performs a relative load, that
/// importer is consulted first with a URL resolved against the containing
/// stylesheet's canonical URL.
@NotNullByDefault
public non-sealed interface SassContentsImporter extends SassImporter {
    ...
    /// Converts a requested URL into the URL that uniquely identifies the
    /// stylesheet.
    ///
    ...
    /// deprecation, and passes the same relative URL to [#load(URI)]. Once a
    /// URL is returned, this importer's [#load(URI)] method is used and no later
