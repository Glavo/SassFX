...
- Matches JavaFX's multiline ordinary-string behavior and applies shared
  whitespace, block-comment, and line-comment trivia rules to declaration
  values, imports, media conditions, and functional pseudo-classes. Comments
  that would consume a following token or stylesheet boundary are rejected.
